...
public class GraphicLayer
{
    ...
    public Coroutine SetTexture(string filePath, float transitionSpeed = 1f, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        if (tex == null)
        {
            Debug.LogError($"Could not load graphic texture from path '{filePath}'. Please ensure it exists within the Resources!");
            return null;
        }
    ...
    public Coroutine SetVideo(string filePath, float transitionSpeed = 1f, bool useAudio = true, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        if (clip == null)
        {
            Debug.LogError($"Could not load graphic video from path '{filePath}'. Please ensure it exists within the Resources!");
            return null;
        }
